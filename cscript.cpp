...
int main() {
    const char* input_file = "singapore.grib2";
    const char* output_file = "output.xml";
    ...
    while ((h = codes_handle_new_from_file(nullptr, f, PRODUCT_GRIB, &err)) != nullptr) {
        ...
        proj->SetAttribute("Lat1", std::to_string(lat1).c_str());
        proj->SetAttribute("Lon1", std::to_string(lon1).c_str());
        proj->SetAttribute("Flags", "128");
        proj->SetAttribute("Lat2", std::to_string(lat2).c_str());
        proj->SetAttribute("Lon2", std::to_string(lon2).c_str());
